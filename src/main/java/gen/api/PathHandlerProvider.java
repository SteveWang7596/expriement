    ...
    * retain any state between multiple sessions.</p>
    ...
    public HttpHandler getHandler(final String basePath) {
        return Handlers.routing()
            .add(Methods.POST, basePath + "/v1/AcceptorAuthorisation", new HttpHandler() {
                @Override
                public void handleRequest(HttpServerExchange exchange) throws Exception {
                    authorisation().handleRequest(exchange);
                }
            })
            ;
    }
    ...
    public HttpHandler getStatefulHandler(final String basePath) {
        return Handlers.routing()
            .add(Methods.POST, basePath + "/v1/AcceptorAuthorisation", authorisation())
            ;
    }
}
